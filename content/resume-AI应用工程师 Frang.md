---
title: "个人简历-AI应用工程师"
date: 2025-01-18
draft: false
layout: "single"
url: "/resume-ai/"
description: "Frang的个人简历 - AI应用工程师，10余年Java/Python后端功底，两年LLM应用一线落地：模型网关、RAG知识库、IM智能助手"
keywords: ["简历", "AI应用工程师", "LLM", "RAG", "Agent", "LangChain", "大模型", "SSE", "FastAPI", "分布式", "Frang"]
---

# Frang

**AI应用工程师**

📧 Frang998877@gmail.com ｜ ✨ 英文名：Frang ｜ 📍 支持远程/现场

---

## 概述

10年后端研发经验，近两年专注大模型应用工程化落地。主导建设大模型统一网关（OpenAI 兼容协议、SSE 流式输出、多 Key 轮换与配额管理），负责 IM 产品 AI 智能助手集成（单聊问答与群聊 @AI 触发场景），并基于 RAG 技术将内部文档构建为可检索、可溯源的知识库问答系统。擅长将原型能力转化为生产级稳定服务，在超时分级控制、重试退避、服务降级、Token 成本核算等工程化环节具备完整的实践经验。

---

## 技术面板

**AI 侧**

- 模型接入：OpenAI 兼容协议、Chat/Embedding 全量调用、SSE 流式解析与断连重试
- Agent：Function Calling 编排、参数校验、失败兜底；了解 Semantic Kernel 插件机制
- RAG：LangChain（Chain/Retrieval）实践 + 自研管线并用，语义切分、向量化、Top-K 召回、带出处的回答（可点回原文）
- Prompt：系统提示词版本化管理、few-shot、结构化输出约束

**工程侧**

- 语言：Python（FastAPI）为主，Java（Spring Boot/Netty）
- 存储/中间件：MySQL、Redis、Kafka、RocketMQ、Elasticsearch、EMQX
- 稳定性：队列+信号量并发闸门、指数退避、多模型多 Key 热备、Token 成本核算
- 底子：数据结构与算法、操作系统、计算机网络；Linux/Shell/Python 运维脚本

**工具链**

- 日常重度使用 Claude Code  等 AI 编程工具，有实际的 Agent 搭建经验

---

## 项目经验


### 💬 IM 智能助手

2024.11 - 2025.08 ｜ 角色：AI 功能实现者

在即时通讯产品里落地 AI 助手，单聊直答、群聊 @触发。

- 消息链路：@识别 → 上下文组装 → 网关调用 → 流式回包，全链路自研
- 防滥用三道闸：服务层禁言/踢人、网关拦截、前端隐藏，恶意刷量有效拦截
- 提示词按场景分版本管理，人设切换不串味

AI 功能从 0 到 1 上线并成为日常入口；复用网关通道，业务侧不直接碰模型 API。

### 📚 内部知识库问答

2025.09 - 2026.02 ｜ 角色：功能实现者

把规范、FAQ、运维手册做成能问答的知识库。

- 文档管线：Markdown/PDF 解析，按语义段落切分，保留标题层级
- 检索管线：Embedding + Top-K 召回，相似度阈值过滤
- 可信度：回答附来源引用，原文可核对，压幻觉
- 迭代法：badcase 归集驱动提示词与切分调参，命中率逐步爬升

上线后规范类问题基本不再靠人工答疑。

---

## 工作履历

### 🏢 顺丰科技 ｜ 高级后端工程师（AI 方向）

2021.03 - 2026.03（5 年）

核心系统开发与架构，2024 年起牵头大模型在 IM 与直播业务落地：AI 网关、智能助手、知识库问答全部从 0 到 1 上线；制定团队 AI 接入技术规范并推广。

技术栈：Java/Spring、Python/FastAPI、MySQL、Redis、EMQX、SSE

### 🏢 微云科技 ｜ 后端工程师

2014.12 - 2020.02（5 年）

企业级 SaaS 平台（200+ 企业客户）核心模块开发：表结构与索引优化、SQL 审核、分布式缓存落地、通用组件库建设。

技术栈：Spring Cloud、MyBatis、MySQL、Redis、Elasticsearch

---

## 🎓 教育

**计算机科学与技术 ｜ 本科**

---

## 📮 联系

- 📧 Frang998877@gmail.com
- ✨ 英文名：Frang（备注：招聘/技术交流）
- 📍 支持远程/现场

💰 面议 ｜ 🕰️ 随时到岗 ｜ 🤝 全职
